...
def load_models():     
    models = {
        '1': {'KNN': joblib.load('model1KNN.joblib')},
        '2': {'RF': joblib.load('model2RF.joblib')},
        '4': {'LR': joblib.load('model4LR.joblib')}
    }
    return models
...
def ensemble_phishing_detection(models, X_urls, y_true_text, vectors_dict):
    y_pred_ensemble = []
    y_true = np.array([1 if label == 'phishing' else 0 for label in y_true_text])  # 1 for phishing, 0 for legitimate
    
    for url_idx in tqdm(range(len(X_urls)), desc="Ensemble prediction"):
        votes = 0
        
        # Model 1 KNN (parser 1, pred == -1 is phishing)
        vector1 = vectors_dict['1'][url_idx]
        if vector1 is not None:
            pred1 = models['1']['KNN'].predict(np.array([vector1]))[0]
            if pred1 == -1:
                votes += 1
        
        # Model 2 RF (parser 2, pred == 1 is phishing)
        vector2 = vectors_dict['2'][url_idx]
        if vector2 is not None:
            pred2 = models['2']['RF'].predict(np.array([vector2]))[0]
            if pred2 == 1:
                votes += 1
        
        # Model 4 LR (parser 4, pred == 1 is phishing)
        vector4 = vectors_dict['4'][url_idx]
        if vector4 is not None:
            pred4 = models['4']['LR'].predict(np.array([vector4]))[0]
            if pred4 == 1:
                votes += 1
        
        # Majority vote: if 2 or more votes for phishing
        is_phishing = 1 if votes >= 2 else 0
        y_pred_ensemble.append(is_phishing)
    
    ensemble_acc = accuracy_score(y_true, y_pred_ensemble)
    print(f"\nEnsemble accuracy (model1KNN + model2RF + model4LR, majority vote >=2 for phishing): {ensemble_acc:.4f}")
...
def run_evaluation():
    ...
    X_urls, y_true_text = prepare_data(df)

    models = load_models()

    vectors_dict = extract_features(X_urls)
    
    ensemble_phishing_detection(models, X_urls, y_true_text, vectors_dict)
