...
def measure_time_response(url):
    ...
    try: 
        ...
        logging.error(f"Unexpected error in measure_time_response for {url}")
        return -1
    end_time = time.time()
...
def spf_domain(domain):
    ...
    try:
        ...
        features['domain_spf'] = 1 if spf_found else 0
        return
        ...
        logging.error(f"Unexpected error in spf_domain for {domain}")
        features['domain_spf'] = -1
        return
...
def asn_ip(string):
    ...
    try:
        ...
        logging.error(f"Unexpected error in asn_ip for {string}")
        features["asn_ip"]=-1
        return 
...
def time_domain(string):
    try:         
        ...
        logging.error(f"Unexpected error in time_domain for {string}")
        features['time_domain_activation'] = -1
        features['time_domain_expiration'] = -1
        return
...
def qty_ip_resolved(domain):      
    try:
        ...
        logging.error(f"Unexpected error in qty_ip_resolved for {domain}")
        features['qty_ip_resolved']=-1
        return 
...
def qty_nameservers(domain):
    try:
        ...
        logging.error(f"Unexpected error in qty_nameservers for {domain}")
        features['qty_nameservers'] = -1
        return
...
def qty_mx_servers(domain):
    try:
        ...
        logging.error(f"Unexpected error in qty_mx_servers for {domain}")
        features['qty_mx_servers'] = -1
        return
...
def ttl_hostname(domain):
    try:
        ...
        logging.error(f"Unexpected error in ttl_hostname for {domain}")
        features['ttl_hostname'] = -1
        return
...
def tls_ssl_certificate(domain):
    try:
        ...
        logging.error(f"Unexpected error in tls_ssl_certificate for {domain}")
        features['tls_ssl_certificate'] = -1
        return
...
def qty_redirects(url):
    ...
    try:
        ...
        logging.error(f"Unexpected error in qty_redirects for {url}")
        features['qty_redirects'] = -1
        return
...
def domain_google_index(domain):
    ...
    try:
        ...
        logging.error(f"Unexpected error in domain_google_index for {domain}")
        features['domain_google_index'] = -1
        return
...
def qty_tld_url(url):
    try:
        ...
        logging.error(f"Unexpected error in qty_tld_url for {url}")
...
def server_client_domain(url):
    try:
        ...
        logging.error(f"Unexpected error in server_client_domain for {url}")
        features["server_client_domain"] = -1
        return
...
def parse_string(string):
    ...
    try:
        socket.getaddrinfo(domain_string, None)  
    except socket.gaierror:
